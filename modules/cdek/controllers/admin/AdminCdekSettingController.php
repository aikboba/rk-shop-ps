...
class AdminCdekSettingController extends ModuleAdminController
{
    ...
    public function setMedia($isNewTheme = false)
    {
        ...
        $this->context->controller->addCSS(array(
            $this->module->getPathUri().'views/css/bootstrap-clockpicker.min.css',
        ...
        $this->context->controller->addJS(array(
            $this->module->getPathUri().'views/js/bootstrap-clockpicker.min.js',
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitAddconfiguration')) {
            ...
            $company = Tools::getValue(ConfSK::formatConfName('company'));
            if (!empty($company)) {
                ConfSK::setConf('company', Tools::getValue(ConfSK::formatConfName('company')));
            } else {
                $this->errors[] = $this->l('company empty!');
            }

            $name = Tools::getValue(ConfSK::formatConfName('name'));
            if (!empty($name)) {
                ConfSK::setConf('name', Tools::getValue(ConfSK::formatConfName('name')));
            } else {
                $this->errors[] = $this->l('name empty!');
            }
            $address = Tools::getValue(ConfSK::formatConfName('address'));

            if (!empty($address)) {
                ConfSK::setConf('address', Tools::getValue(ConfSK::formatConfName('address')));
            } else {
                $this->errors[] = $this->l('address empty!');
            }

            $street = Tools::getValue(ConfSK::formatConfName('street'));
            if (!empty($street)) {
                ConfSK::setConf('street', Tools::getValue(ConfSK::formatConfName('street')));
            } else {
                $this->errors[] = $this->l('street empty!');
            }

            $house = Tools::getValue(ConfSK::formatConfName('house'));
            if (!empty($house)) {
                ConfSK::setConf('house', Tools::getValue(ConfSK::formatConfName('house')));
            } else {
                $this->errors[] = $this->l('house empty!');
            }

            $flat = Tools::getValue(ConfSK::formatConfName('flat'));
            if (!empty($flat)) {
                ConfSK::setConf('flat', Tools::getValue(ConfSK::formatConfName('flat')));
            } else {
                $this->errors[] = $this->l('flat empty!');
            }

            $phone = Tools::getValue(ConfSK::formatConfName('phone'));
            if (!empty($phone)) {
                ConfSK::setConf('phone', Tools::getValue(ConfSK::formatConfName('phone')));
            } else {
                $this->errors[] = $this->l('phone empty!');
            }

            $start_time = Tools::getValue(ConfSK::formatConfName('start_time'));
            if (!empty($start_time)) {
                ConfSK::setConf('start_time', Tools::getValue(ConfSK::formatConfName('start_time')));
            } else {
                $this->errors[] = $this->l('start_time empty!');
            }

            $end_time = Tools::getValue(ConfSK::formatConfName('end_time'));
            if (!empty($end_time)) {
                ConfSK::setConf('end_time', Tools::getValue(ConfSK::formatConfName('end_time')));
            } else {
                $this->errors[] = $this->l('end_time empty!');
            }
// end add
...
//            );
            ...
            ConfSK::setConf('AddService', (int)Tools::getValue(ConfSK::formatConfName('AddService')));
    ...
    public function getFieldsValue($obj)
    {
        ...
        //        add
        $fields_value[ConfSK::formatConfName('company')] = ConfSK::getConf('company');
        $fields_value[ConfSK::formatConfName('name')] = ConfSK::getConf('name');
        $fields_value[ConfSK::formatConfName('address')] = ConfSK::getConf('address');
        $fields_value[ConfSK::formatConfName('street')] = ConfSK::getConf('street');
        $fields_value[ConfSK::formatConfName('house')] = ConfSK::getConf('house');
        $fields_value[ConfSK::formatConfName('flat')] = ConfSK::getConf('flat');
        $fields_value[ConfSK::formatConfName('phone')] = ConfSK::getConf('phone');
        $fields_value[ConfSK::formatConfName('start_time')] = ConfSK::getConf('start_time');
        $fields_value[ConfSK::formatConfName('end_time')] = ConfSK::getConf('end_time');
        ...
        $fields_value[ConfSK::formatConfName('AddService')] = (float)ConfSK::getConf('AddService');
        ...
        $this->fields_form = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'name' => ConfSK::formatConfName('company'),
                    'type' => 'text',
                    'required' => true
                ),
                array(
                    'label' => $this->l('Name'),
                    'name' => ConfSK::formatConfName('name'),
                    'type' => 'text',
                    'required' => true
                ),
                array(
                    'label' => $this->l('Address'),
                    'name' => ConfSK::formatConfName('address'),
                    'type' => 'text',
                    'required' => true
                ),
                array(
                    'label' => $this->l('Street'),
                    'name' => ConfSK::formatConfName('street'),
                    'type' => 'text',
                    'required' => true
                ),
                array(
                    'label' => $this->l('House'),
                    'name' => ConfSK::formatConfName('house'),
                    'type' => 'text',
                    'required' => true
                ),
                array(
                    'label' => $this->l('Flat'),
                    'name' => ConfSK::formatConfName('flat'),
                    'type' => 'text',
                    'required' => true
                ),
                array(
                    'label' => $this->l('Phone'),
                    'name' => ConfSK::formatConfName('phone'),
                    'type' => 'text',
                    'required' => true
                ),
                array(
                ...
                array(
                    'label' => $this->l('AddService'),
                    'name' => ConfSK::formatConfName('AddService'),
                    'type' => 'switch',
                    'values' => array(
                        array(
                            'id' => ConfSK::formatConfName('AddService').'_on',
                            'value' => 1,
                            'label' => $this->l('Yes')
                        ),
                        array(
                            'id' => ConfSK::formatConfName('AddService').'_off',
                            'value' => 0,
                            'label' => $this->l('No')
                        )
                    )
                ...
                array(
                    ...
                    'name' => ConfSK::formatConfName('start_time'),
                    'type' => 'time',
                    'required' => true
                ),
                array(
                    'label' => $this->l('End time'),
                    'name' => ConfSK::formatConfName('end_time'),
                    'type' => 'text',
                    'placeholder' => '117623',
                    'required' => true
                ),
                ...
                array(
                    'label' => $this->l('Tax for additional services'),
