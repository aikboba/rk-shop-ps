...
class CDEK extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->config = array(
            ...
            'weight' => 1,
            'categories_setting' => '',
            'DeliveryRecipientVATRate' => 'VAT20',
            'carriers_settings' => '',
            'weight_unit' => 1,
            'status_send_order' => 2,
...
//                }
        ...
        if (Validate::isLoadedObject($order)) {
            ...
            if (Validate::isLoadedObject($order)
                && (in_array($order->current_state, FormatConfOrderState::getCancelOrderStatuses())
                    || in_array($order->current_state, FormatConfOrderState::getSendOrderStatuses()))) {
                $order_info = $this->getOrderInfo($order);
                if ($order_info['DispatchNumber']) {
                    $id_order_carrier = $order->getIdOrderCarrier();
                    $order_carrier = new OrderCarrier($id_order_carrier);
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        if (Validate::isLoadedObject($order)
            ...
            foreach (FormatConfOrderState::getSendOrderStatuses() as $id_order_state) {
                ...
                $statuses[$id_order_state] = $order_state->name;
            }
            ...
            if ($tariff) {
                ...
                $this->context->smarty->assign(array(
                    ...
                    'status_for_print' => key_exists($order->current_state, $statuses)
                ));
    ...
    public function getOrderInfo($order)
    {
        ...
        if ($history_response && ToolsModuleSK::checkItemArray('StatusReport', $history_response)) {
            if (ToolsModuleSK::checkItemArray('@attributes', $history_response['StatusReport'])) {
                if (ToolsModuleSK::checkItemArray('ErrorCode', $history_response['StatusReport']['@attributes'])) {
        ...
        if ($history_response && ToolsModuleSK::checkItemArray('StatusReport', $history_response)) {
            if (!ToolsModuleSK::checkItemArray('Order', $history_response['StatusReport'])) {
                $history_response = false;
            }
        }
...
//            $headers = get_headers('https://integration.cdek.ru/status_report_h.php');
//            if(substr($headers[0], 9, 3) > 400)
//                return true;
            $host = 'https://integration.cdek.ru/status_report_h.php';
            $port = 80;
            $timeout = 6;
            ...
            if($response['StatusReport']['Order']['@attributes']['ErrorCode'] == 'ERR_ORDER_NOTFIND'){
                return true;
            }
...
//                    $response['result']['deliveryPeriodMax']);
            ...
            // format date
            $response['result']['deliveryDateMin'] = explode("-", $response['result']['deliveryDateMin']);
            $response['result']['deliveryDateMin'] = array_reverse($response['result']['deliveryDateMin']);
            $response['result']['deliveryDateMin'] = implode(".", $response['result']['deliveryDateMin']);

            $response['result']['deliveryDateMax'] = explode("-", $response['result']['deliveryDateMax']);
            $response['result']['deliveryDateMax'] = array_reverse($response['result']['deliveryDateMax']);
            $response['result']['deliveryDateMax'] = implode(".", $response['result']['deliveryDateMax']);
            ...
            if ($response['result']['deliveryDateMin'] == $response['result']['deliveryDateMax'])
                ...
                $date = sprintf($this->l('Estimated delivery time from %s to %s.'),
                    '<span class="delivery_date_min">'.$response['result']['deliveryDateMin'].'</span>',
                    '<span class="delivery_date_max">'.$response['result']['deliveryDateMax'].'</span>');
